...
class GraphContainer(Container):
    ...
    def do_layout(self, size=None, force=False):
        ...
        def _apply_graphviz_layout(layout):
            min_x = min([pos[0] for pos in list(layout.values())])
            max_y = max([pos[1] for pos in list(layout.values())])
        ...
        if all(point == [0.0, 0.0]
               for point in list(initial_positions.values())):
        ...
        if self.style == 'tree':
            try:
                ...
                depths = [v[1] for v in list(layout.values())]
    ...
    def draw(self, gc, view_bounds=None, mode="default"):
        ...
        if self.graph.is_directed():
            with gc:
                ...
                for (x, y), (dx, dy) in zip(line_starts,
            ...
            if len(vec) == 0:
                return
            unit_vec = vec / numpy.sqrt(vec[:, 0]**2 + vec[:, 1]**
                                        2)[:, numpy.newaxis]
            ...
            with gc:
                ...
                arrow_ends = line_ends - numpy.array(unit_vec * numpy.matrix(
                    [[c, s], [-s, c]])) * 10
                gc.begin_path()
